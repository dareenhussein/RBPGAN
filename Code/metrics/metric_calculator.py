...
class MetricCalculator():
    ...
    def compute_SSIM(self):
        ...
        true_img = self.true_img_cur
        pred_img = self.pred_img_cur
        if not true_img.shape == pred_img.shape:
            raise ValueError('Input images must have the same dimensions.')
        if true_img.ndim == 2:
            return self.SSIM(true_img, pred_img)
        elif true_img.ndim == 3:
            if true_img.shape[2] == 3:
                ssims = []
                for i in range(3):
                    ssims.append(self.SSIM(true_img, pred_img))
                return np.array(ssims).mean()
            elif true_img.shape[2] == 1:
                return self.SSIM(np.squeeze(true_img), np.squeeze(pred_img))
        else:
            raise ValueError('Wrong input image dimensions.')

    def SSIM(self, true_img, pred_img):
        C1 = (0.01 * 255)**2
        C2 = (0.03 * 255)**2

        true_img = true_img.astype(np.float64)
        pred_img = pred_img.astype(np.float64)
        kernel = cv2.getGaussianKernel(11, 1.5)
        window = np.outer(kernel, kernel.transpose())

        mu1 = cv2.filter2D(true_img, -1, window)[5:-5, 5:-5]  # valid
        mu2 = cv2.filter2D(pred_img, -1, window)[5:-5, 5:-5]
        mu1_sq = mu1**2
        mu2_sq = mu2**2
        mu1_mu2 = mu1 * mu2
        sigma1_sq = cv2.filter2D(true_img**2, -1, window)[5:-5, 5:-5] - mu1_sq
        sigma2_sq = cv2.filter2D(pred_img**2, -1, window)[5:-5, 5:-5] - mu2_sq
        sigma12 = cv2.filter2D(true_img * pred_img, -1, window)[5:-5, 5:-5] - mu1_mu2
